...
function ENT:SpawnFunction( ply, tr )
	...
	local ent = ents.Create( "deuterium_amatter_conv" )
...
function ENT:Initialize()
	...
	if not (WireAddon == nil) then
		...
		self.Outputs = Wire_CreateOutputs(self.Entity, { "Energy Input", "Deuterium Input", "antimatter Output", "On" })
	end
end
...
function ENT:Convert()
	if (self.Active == 1 ) then
		...
		if (self.deuterium >= Deuterium_Increment) and (self.energy >= Energy_Increment) then
			...
			RD.SupplyResource(self.Entity, "antimatter", antimatter_Increment)
		end
	else
		self:TurnOff()
	end
...
function ENT:UpdateWireOutput()
	...
	Wire_TriggerOutput(self.Entity, "Anti Deuterium Output", antimatter)
